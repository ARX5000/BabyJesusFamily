...
<div class="max-w-lg mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
        <h2 class="text-xl font-bold text-slate-800 mb-6">تعديل: {{ $class->name }}</h2>
        ...
        <form method="POST" action="{{ route('admin.classes.update', $class) }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $class->name) }}" class="form-input">
                @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="form-label">الوصف</label>
                <textarea name="description" rows="3" class="form-input">{{ old('description', $class->description) }}</textarea>
